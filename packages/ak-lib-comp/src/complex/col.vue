<template>
  <div  :class='{
      "ivu-form-item":true, 
      "ivu-form-item-required": isError||isRequire,
      "ivu-form-item-error":isError
      }'>
    <label class="ivu-form-item-label" style="width: 90px;">{{displayName}}</label>
    <div class="ivu-form-item-content" style="margin-left: 90px;">
       ...
       <DatePicker   @on-change="_dateTimeSpanChange"   v-else-if="ControlType=='DateTime'"  format="yyyy-MM-dd HH:mm:ss"   type="datetimerange" placement="bottom-start" placeholder="Select date" style="width: 350px"></DatePicker>
        ...
        <div  v-if="isError" class="fade-transition ivu-form-item-error-tip">{{ErrorMesg}}</div>
    </div>
</div>
</template>
