            ...
            <div class="banner2">
                ...
                <Form :label-width="120" ref="currentForm" :model="currentForm" :rules="rulesValidation">
                    ...
                    <Form-item>
                        ...
                        <Button type="ghost" @click="debug" style="margin-left: 8px">debug</Button>
                    </Form-item>
                </Form>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                methodName: '添加',
                taskOptions: [
                    {key: 'name', title: '任务名', typ: 'readonly'},
                    {key: 'owner_cnname', title: '责任人'},
                    {key: 'alarm_config', title: '报警设置', typ: 'textarea'}
                ],
                taskForm: {
                    id: 0,
                    name: '',
                ...
                currentForm: {},
                rulesValidation: {
                    owner_cnname: [{required: true, message: '责任人不能为空', type: 'string'}]
                }
            }
        },
        methods: {
            ...
            getTask(task_id) {
                ...
                dwhApi.getTask(params).then(resp => {
                    let error = resp.data.error
                    if (error == 0) {
                        this.taskForm.name = resp.data.data.name
                        this.taskForm.owner_cnname = resp.data.data.owner_cnname
                        this.taskForm.alarm_config = resp.data.data.alarm_config
                    } else {
                        this.$Message.error({
                            content: '获取数据出错! ' + error, onClose: () => {
            ...
            debug() {
                this.$Message.info({
                    content: JSON.stringify(this.taskForm), onClose: () => {
                    }, duration: 2, closable: false
                });
            }
        },
